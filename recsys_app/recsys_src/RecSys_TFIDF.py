...
def get_TFIDF_RecSys(dframe, qu_phrase="kirjasto", user_name=args.qusr, nwp_title_issue_page_name=args.qtip, topN=5):
	...
	print(f">> Cleaning list of all OCR content...")
	st_t = time.time()
	...
	tfidf_vec_fpath = os.path.join(dfs_path, f"{fprefix}_tfidf_vectorizer_nwp_content.lz4")
	tfidf_rf_matrix_fpath = os.path.join(dfs_path, f"{fprefix}_tfidf_matrix_RF_nwp_content.lz4")

	if not os.path.exists(tfidf_rf_matrix_fpath):
		print(f"TFIDF for {len(fst_lst_cleaned)} documents, might take a while...".center(110, " "))
		st_t = time.time()
		...
		tfidf_matrix_rf = tfidf_vec.fit_transform(raw_documents=fst_lst_cleaned)
	...
	tokens = my_tokenizer(qu_phrase)
	print(f">> tokenize query word: '{qu_phrase}'\t {len(tokens)} {tokens}")
